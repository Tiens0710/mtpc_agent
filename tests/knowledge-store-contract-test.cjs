...
assert(zalo.includes("MTPC_ZALO_AGENT_BUILD', 'agent-zalo-v13"), 'Zalo Agent build marker must identify the conversation-aware admissions release');
assert(zalo.includes("require_once __DIR__ . '/zalo-conversation.php'"), 'Zalo Agent must load its per-user conversation helpers');
assert(zalo.includes('mtpc_zalo_agent_history_contents($history, $question)') && zalo.includes("'contents' => $contents"), 'Gemini must receive the bounded multi-turn transcript');
assert(zalo.includes('mtpc_zalo_agent_history_contextual_query($question, $history)'), 'knowledge retrieval must resolve follow-up questions from recent user context');
assert(zalo.includes('mtpc_zalo_agent_history_is_conversion_detail_followup') && zalo.includes('mtpc_zalo_agent_history_chunk_has_current_conversion_detail'), 'conversion duration follow-ups must reject unrelated or historical evidence');
assert(zalo.includes('Lịch sử chat không phải bằng chứng') && zalo.includes('không lấy câu trả lời trước của bạn làm nguồn'), 'conversation history may resolve references but never serve as factual evidence');
assert(zalo.includes("mtpc_zalo_agent_history_append($userId, 'user', $text") && zalo.includes("mtpc_zalo_agent_history_append($userId, 'model', $reply"), 'Zalo messages must be stored only for that user and assistant replies only after send');
